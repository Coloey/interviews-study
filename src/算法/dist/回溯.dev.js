...
*/
var letterCombanations = function letterCombanations(digits) {
  ...
  var dfs = function dfs(digits, index) {
    ...
    var letterIndex = +digits[index]; // 拿到字母

    var letter = letterMap[letterIndex]; // 遍历的是不同的集合 所以i可以从0开始
...
var conbine1 = function conbine1(n, k) {
  var res = [];
  var path = [];

  var dfs = function dfs(n, start) {
    if (path.length === k) {
      res.push(path.slice());
      return;
    }
...
var conbine2 = function conbine2(n, k) {
  var res = [];
  var path = [];
  var sum = 0;
...
var conbine3 = function conbine3(candidates, target) {
  candidates.sort(function (a, b) {
    return a - b;
  });
  var res = [];
  var path = [];
  var used = new Array(candidates.length).fill(false);
  var sum = 0;
...
var sortQue = function sortQue(nums) {
  var res = [];
  var path = [];
...
var sortQue2 = function sortQue2(nums) {
  var res = [];
  var path = [];
  ...
  var dfs = function dfs() {
    ...
    for (var i = 0; i < nums.length; i++) {
      if (i > 0 && nums[i] === nums[i - 1] && used[i - 1] === false) continue; // 没有start,所以为了避免重复读取
...
var findSubsequence = function findSubsequence(nums) {
  var res = [];
  var path = [];
