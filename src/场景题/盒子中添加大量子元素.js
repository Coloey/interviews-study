...
//使用 CSS3 transform：将子元素设置为绝对定位，并使用 CSS3 transform 属性进行变换。这样可以将所有子元素合成为一层，
//减少重绘和重排的次数，提高性能
/*.parent{
    position:relative;
}
...
function addBatch(){
    let fragment = document.createDocumentFragment()
    for(let i=0;i<batchSize && currIndex<totalCount;i++){
        let childElement=document.createElement('div')
        childElement.textContent='Child'+currIndex;
        fragment.appendChild(childElement)
        currIndex++;
    }
