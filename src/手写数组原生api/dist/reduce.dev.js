...
Array.prototype.myReduce = function (callback, initValue) {
  if (typeof callback !== "function") {
    throw new Error("".concat(callback, " is not a function"));
  }

  var arr = this;
  var preValue = initValue;
  var index = 0,
      len = arr.length;
...
function reduce(fn, initValue) {
  ...
  var arr = this;
  var acc;
  var initIndex; // 如果没有存入初始值，initval为数组第一项

  initIndex = arguments.length === 1 ? 1 : 0; // 没有传入初始值 累加器为数组第一位元素
...
var res = arr._reduce(function (pre, cur) {
  return pre + cur;
}, 0);